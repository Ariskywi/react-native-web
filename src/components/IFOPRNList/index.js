...
class IFOPRNList<ItemT> extends React.PureComponent<Props<ItemT>, void> {
    ...
    componentWillReceiveProps(nextProps: Props<ItemT>) {
        invariant(
            ...
            'Changing numColumns on the fly is not supported. Change the key prop on IFOPRNList when ' +
    ...
    _checkProps(props: Props<ItemT>) {
        ...
        invariant(
            ...
            'IFOPRNList does not support custom data formats.',
        );
        if (numColumns > 1) {
        ...
        if (legacyImplementation) {
            ...
            if (!this._hasWarnedLegacy) {
                console.warn(
                    'IFOPRNList: Using legacyImplementation - some features not supported and performance ' +
                    'may suffer',
                );
    ...
    _keyExtractor = (items: ItemT | Array<ItemT>, index: number) => {
        ...
        if (numColumns > 1) {
            invariant(
                Array.isArray(items),
                'IFOPRNList: Encountered internal consistency error, expected each item to consist of an ' +
